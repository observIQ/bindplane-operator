...
var _ = Describe("Bindplane workloads", Ordered, Label(ginkgoLabelRequiresLicense), func() {
	...
	It("reconciles a minimal Bindplane custom resource into managed workloads", func() {
		...
		Expect(bindplane.Status.Components.JobsMigrate.Image).To(Equal(expectedEEImage(bindplane.Spec.Version)))
		Expect(bindplane.Status.Components.Bindplane.ReadyReplicas).To(BeNumerically(">", 0))
		Expect(bindplane.Status.Components.Nats.ReadyReplicas).To(BeNumerically(">", 0))
		Expect(bindplane.Status.Components.TransformAgent.ReadyReplicas).To(BeNumerically(">", 0))
