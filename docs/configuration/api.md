...
| `podDisruptionBudget` _[PodDisruptionBudgetSpec](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#poddisruptionbudgetspec-v1-policy)_ | PodDisruptionBudget configures the operator-managed PodDisruptionBudget for this component,<br />using the upstream policy/v1 PodDisruptionBudgetSpec<br />(https://pkg.go.dev/k8s.io/api/policy/v1#PodDisruptionBudgetSpec). The operator sets the<br />selector, so it must be omitted. When omitted, the operator creates a PDB with<br />minAvailable: 1. Ignored when disablePodDisruptionBudget is true. |  | Optional: \{\} <br /> |
