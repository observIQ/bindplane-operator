...
|------|--------------------|---------------------------|
| **License** | `spec.config.license` or `licenseSecretRef` | Injects `BINDPLANE_LICENSE` from value or Secret into Node, Jobs, and the Jobs Migrate Job. The NATS StatefulSet does not receive the license — it runs only the embedded NATS event bus and carries no Bindplane-application config. |
...
- The Bindplane application workloads (Node, Jobs, and the Jobs Migrate Job) receive the same username and password via **`BINDPLANE_PROMETHEUS_AUTH_USERNAME`** and **`BINDPLANE_PROMETHEUS_AUTH_PASSWORD`** (from that Secret) so they can authenticate when using the remote write client to send agent throughput and health metrics to the TSDB. The NATS StatefulSet does not send metrics to the TSDB and does not receive these credentials. These are **not** the same as `BINDPLANE_METRICS_PROMETHEUS_USERNAME` / `BINDPLANE_METRICS_PROMETHEUS_PASSWORD`, which configure basic auth for the endpoint where Bindplane **exposes** its own metrics (see [Configuration – Metrics](configuration.md#metrics)).
...
Bindplane's status check endpoints are enabled by default (see [Configuration – Status checks](configuration.md#status-checks)) and are authenticated with one or more UUID keys.

- When status is enabled and you supply neither `spec.config.status.keys` nor `spec.config.status.keysSecretRef`, the operator generates a UUID and stores it in a Secret named **`<bindplane-name>-status-secret`** under the key `status-keys`.
- The Secret is **created once** when it does not exist; the operator does not update its data on later reconciles, so the key is stable.
- Bindplane workloads reference it via **`BINDPLANE_STATUS_KEYS`** (`valueFrom: secretKeyRef`); the operator process never reads the value.
- Supplying your own `keys` (inline) or `keysSecretRef` overrides the generated Secret — the pods use your value and the operator stops managing the generated Secret. Setting `spec.config.status.enabled: false` disables the endpoints and generates no Secret.
...
- **Secret access** — the operator has `get;list;watch;create;update;patch;delete` on `secrets` in the operator namespace. This is required to create and read the operator-managed Secrets (`<name>-session-secret`, `<name>-status-secret`, and `<name>-tsdb-basic-auth`). The operator does **not** read user-supplied Secrets by value — sensitive field values are injected into pods via `valueFrom: secretKeyRef` (Kubernetes resolves those at pod scheduling time), so the operator process itself never sees plaintext credential values from user Secrets.
...
| Status check keys | Yes (optional override; auto-generated otherwise) | `BINDPLANE_STATUS_KEYS` (`BINDPLANE_STATUS_ENABLED`) | `spec.config.status.keys` / `keysSecretRef` (operator generates `<name>-status-secret` when omitted) | [Configuration – Status checks](configuration.md#status-checks) |
