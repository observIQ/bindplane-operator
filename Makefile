...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name bindplane-operator-builder
	$(CONTAINER_TOOL) buildx use bindplane-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm bindplane-operator-builder
	rm Dockerfile.cross
...
.PHONY: deploy
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config. Uses server-side apply to avoid CRD annotation size limit (262144 bytes).
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
